...
@pytest.fixture
def mock_db_manager():
    """Mock database init for CLI tests."""
    mock_dm = MagicMock()
...
@patch("codegraph.cli.commands.build.setup_logging")
...
@patch("codegraph.cli.commands.doctor.setup_logging")
def test_doctor_helper_logic_success(
    ...
    mock_get_dm.return_value = mock_db_manager
    ...
    with patch("codegraph.core.graph.ppr.create_gds_client") as mock_gds_client:
        ...
        doctor_helper(Path("config.yaml"))
