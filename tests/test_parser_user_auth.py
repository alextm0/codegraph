...
def parser():
    ...
    Returns:
        parser: A configured parser instance for parsing Python source into file entities.
    """
...
def read(relative_path: str) -> bytes:
    """
    Read a file from the test fixture project.
    
    Parameters:
        relative_path (str): Path to the file within the fixture project, relative to the PROJECT root.
...
def test_absolute_imports_extracted_with_correct_module_paths(parser):
    """
    Verify that parsing services/auth_service.py extracts the absolute project import module paths.
    
    Asserts the parser yields import module_path values {"user_auth.models.user", "user_auth.utils.validators"}.
    """
...
def test_intra_file_call_is_captured(parser):
    """
    Ensure a call from `validate_password` to `validate_password_strength` is recorded in the parsed entities.
    
    Also asserts that the recorded call has a `line_number` greater than or equal to 1.
    """
    entities = parse_file(read("utils/validators.py"), "validators.py", parser)
