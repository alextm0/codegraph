...
\begin{enumerate}
    ...
    \item \textbf{Semantic Gaps (38 instances):} These failures arise when the issue description uses abstract natural-language terms with insufficient lexical overlap with the target code identifiers. For example, in \textit{django/django-10914}, a request to change default file permissions mentions high-level concepts like ``tempfile'' and ``permissions'' but does not name the target configuration hub, \texttt{global\_settings.py}. Without a precise lexical match to the correct starting neighborhood, structural propagation has no entry point to reach the logically related but structurally isolated target.
\end{enumerate}
