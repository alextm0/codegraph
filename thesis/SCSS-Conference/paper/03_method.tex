...
CodeGraph treats code retrieval as a two-stage process: lexical signals prioritize the entry points (seeds) into the codebase, while structural propagation disambiguates and ranks the broader functional neighbourhood. This division of labour ensures that natural-language queries can find a starting point, while the ranking itself remains anchored in execution semantics.
...
To attenuate the influence of generic hubs, CodeGraph assigns each edge a weight inversely proportional to the in-degree of its target node: $w(e) = 1 / \log_2(\operatorname{in\_degree}(v) + 2)$. This logarithmic penalty steers probability mass away from ubiquitous hubs, ensuring that relevance is propagated primarily along domain-specific pathways. \Cref{fig:idf-reweighting} illustrates this intuition.
...
The methodology is governed by several strict design choices that prioritize robust structural semantics over raw data density or empirical heuristics.
